...
export default class Validation {
   ...
   * 参数验证
   * 此方法主要用于后端API接口的参数验证
   * @param params {object} 包含待验证参数的Map
   * @param validations {object} 包含验证规则的Map。格式示例: {
   ...
   *   }
   * }
   * @param ignoreRequired {boolean|undefined} 是否忽略验证规则中的"Required"验证器
   */
  static validate(params, validations, ignoreRequired) {
    if (typeof params !== 'object' || params === null) {
      throw new Error('参数params必须是object');
    }
    for (let keyPath of Object.keys(validations)) {
      let rules = validations[keyPath];
      let keys = this._compileKeyPath(keyPath);
      this._validate(params, keys, rules, '', ignoreRequired);
    }
  }
  ...
  /**
   * 验证一个值是否符合规则
   * @param value {any} 待验证的值
   * @param rules {string|string[]} 规则。可以是一条规则；也可以是多条规则的数组，任何一条规则验证通过即可
   * @param alias {string} 如果验证规则中包含'Alias:xxx'，则这个参数可以不传；否则一定要传
   * @return {any} 返回value原值
   */
